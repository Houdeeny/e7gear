...
function reset() {
	document.getElementById("atkper").value = "", document.getElementById("critch").value = "", document.getElementById("critdmg").value = "", document.getElementById("hpper").value = "", document.getElementById("defper").value = "", document.getElementById("spd").value = "", document.getElementById("eff").value = "", document.getElementById("res").value = "", document.getElementById("atkflat").value = "", document.getElementById("defflat").value = "", document.getElementById("hpflat").value = "", document.getElementById("gear-type").value = "gpink", document.getElementById("gear-ench").value = "gear-lv0", resetplaceholder(), document.getElementById("score").innerHTML = "0", document.getElementById("score-percentage").innerHTML = "0%", document.getElementById("score-percentage").style = "background-color:#eee", document.getElementById("result").style = "border-style:none", err("")
}
...
function validate() {
	for (var e = getSubstat(), t = getSubstatMin(), r = 0, a = getReforge(0), n = 0; n < 11; n++)
		if (isNaN(e[n]) && (e[n] = 0), e[n] < 0) {
			switch (n) {
				case 0:
					document.getElementById("result").style = "padding:16px; border-style:groove; border-color:#FF4136";
	...
	for (n = 0; n < 11; n++) {
		var c = 0;
		if (!(isNaN(e[n]) || e[n] <= 0) && (r++, c = isReforged() ? t[n] + a[n] : t[n], e[n] > 0 && e[n] < c)) return document.getElementById("result").style = "padding:16px; border-style:groove; border-color:#FF4136", err("ERROR! " + getSubstatName(n) + " (" + e[n] + ") must be larger than " + c), -1
	}
	return r != getRequiredDataCount() ? (document.getElementById("result").style = "padding:16px; border-style:groove; border-color:#FF4136", err("ERROR! This gear should have " + getRequiredDataCount() + " substat(s) according to its grade."), -1) : 0
}
...
function report(e, t) {
	...
	document.getElementById("score").innerHTML = o, document.getElementById("score-percentage").innerHTML = s + "%", s >= 0 && s <= 49 && (document.getElementById("score-percentage").style = "background-color:#FF4136", document.getElementById("result").style = "border-style:groove; border-color:#FF4136"), s >= 50 && s <= 64 && (document.getElementById("score-percentage").style = "background-color:#FFDC00", document.getElementById("result").style = "border-style:groove; border-color:#FFDC00"), s >= 65 && s <= 100 && (document.getElementById("score-percentage").style = "background-color:#2ECC40", document.getElementById("result").style = "border-style:groove; border-color:#2ECC40"), r += "<h4>■ Summary</h4>", f = 1, b = 1;
	for (h = 0; h < d; h++) {
	...
	if (p = f / b == 1 ? 1 : Math.ceil(1 / (f / b)), s >= 70 && getGearEncLevel() > 4) r += "◦ Amazing gear score!";
		...
		r += "◦ Decent roll into " + getSubstatName(m[0]);
	...
	r += "<br>◦ It will take an estimate of " + p + " gears to get a similar one.<br><br>", r = (r += "<h4>■ Gear Score</h4>") + "◦ Gear score is " + o + " out of " + l + " (" + s + "%)<br><br>", r += "<h4>■ Details</h4>";
	for (h = 0; h < d; h++) {
		var E = n[g[h]];
